...
// Récupérer les questions depuis sessionStorage ou utiliser les questions par défaut
let questions = [];

// Au chargement, vérifier s'il y a un quizz stocké
function loadQuiz() {
    const quizData = sessionStorage.getItem('currentQuiz');
    if (quizData) {
        const parsedQuiz = JSON.parse(quizData);
        questions = parsedQuiz.questions;
        document.querySelector('header h1').textContent = `QCM — ${parsedQuiz.title}`;
        sessionStorage.removeItem('currentQuiz'); // Nettoyer après utilisation
    } else {
        // Quizz par défaut si aucun n'est sélectionné depuis le menu
        questions = [
            {
                id: 1,
...
// Charger le quizz et initialiser
loadQuiz();
renderQuestion(currentIndex);
...
document.querySelector('.controls').insertBefore(menuBtn, prevBtn);
